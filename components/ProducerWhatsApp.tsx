...
const ProducerWhatsApp: React.FC<NavigationProps> = ({ onNavigate }) => {
  ...
  const processLogic = (text: string) => {
    ...
    if (flowState === 'MAIN_MENU') {
      if (input === '1' || lower.includes('apoyo')) {
        ...
        if (!pendingDocuments.length) {
            ...
            let m = `📂 *RECAUDOS PENDIENTES*\n\nPor favor, escribe el **número** del documento que deseas subir:\n\n`;
    ...
    if (flowState === 'REQ_PATTERN') {
      ...
      if (activePatterns[idx]) {
        setTempData({ 
            ...
            projectType: activePatterns[idx].projectType 
        });
    ...
    if (flowState === 'REQ_FINCA') {
      ...
      const newApp: CreditApplication = {
        ...
        type: tempData.projectType as 'Consumo' | 'Semilla',
        amount: totalAmount, 
    ...
    // 5. FLUJO CARGA DE DOCUMENTOS
    if (flowState === 'UPLOAD_DOCS') {
      const idx = parseInt(input) - 1;
      if (pendingDocuments[idx]) {
        setSelectedDocId(pendingDocuments[idx].id);
        addBotResponse(`📸 Entendido. Por favor, toca el icono de la cámara o adjunta la **FOTO** de tu:\n\n*${pendingDocuments[idx].name}*`);
      } else {
          addBotResponse('❌ Elige un número válido de la lista de recaudos pendientes.');
      }
      return;
    }
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (!e.target.files?.[0] || !currentProducerId) return;
    
    if (!selectedDocId) {
        addBotResponse('⚠️ Primero debes indicarme qué documento vas a subir escribiendo el número correspondiente de la lista.');
        return;
    }
    ...
    reader.onloadend = () => {
      ...
      const docName = pendingDocuments.find(d => d.id === selectedDocId)?.name || 'Documento';
      
      // Añadir mensaje del usuario con la imagen para feedback visual
      setChatMessages(prev => [...prev, { 
        id: `u-img-${Date.now()}`, 
        text: `Adjunto foto de: ${docName}`, 
        sender: 'user', 
        image: imageData, 
        timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }), 
        status: 'read' 
      ...
      // Actualizar el estado del documento en el contexto global
      updateProducerDocument(currentProducerId, selectedDocId, 'En_Revision', imageData);
      
      // LÓGICA DE CONTINUIDAD
      setTimeout(() => {
          // Buscamos documentos restantes después de esta carga
          const remaining = pendingDocuments.filter(d => d.id !== selectedDocId);

          if (remaining.length > 0) {
              let m = `✅ *¡FOTO RECIBIDA!* He guardado la imagen de *${docName}*.\n\n📂 *AÚN FALTAN RECAUDOS:* Escribe el número del siguiente documento que vas a subir:\n\n`;
              remaining.forEach((d, i) => m += `*${i + 1}*. ⬜ ${d.name}\n`);
              addBotResponse(m);
              setFlowState('UPLOAD_DOCS'); 
          } else {
              addBotResponse(`🎉 *¡EXPEDIENTE DIGITAL COMPLETADO!* 🎉\n\nHas subido todos los recaudos solicitados. Nuestros analistas validarán las fotos y te notificaremos cuando el estatus cambie.`);
              setFlowState('MAIN_MENU');
          }
      ...
      <div className="h-full w-full max-w-md bg-[#e5ddd5] flex flex-col font-sans relative shadow-2xl overflow-hidden">
          ...
          {chatMessages.map((msg, i) => (
            <div key={i} className={`flex ${msg.sender === 'user' ? 'justify-end' : 'justify-start'} animate-in fade-in slide-in-from-bottom-2 duration-300`}>
              <div className={`max-w-[85%] rounded-xl px-3 py-2 shadow-sm relative ${msg.sender === 'user' ? 'bg-[#dcf8c6]' : 'bg-white'}`}>
                {msg.image && (
                  <div className="mb-2 rounded-lg overflow-hidden border border-black/5 bg-slate-100 min-h-[100px] flex items-center justify-center">
                    <img src={msg.image} className="max-h-64 w-full object-contain" alt="Adjunto" />
                  </div>
                )}
        ...
        <div className="bg-[#f0f2f5] px-2 py-2 flex items-center gap-2 border-t border-slate-200">
          <div className="flex-1 bg-white rounded-full flex items-center px-4 py-2 border border-slate-200 shadow-sm focus-within:ring-1 focus-within:ring-[#008069]">
            ...
            />
            <button onClick={() => fileInputRef.current?.click()} className="ml-2 p-1 text-slate-400 hover:text-[#008069] transition-colors">
